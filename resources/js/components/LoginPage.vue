<template>
    <!-- Contentor principal com gradiente de fundo adaptativo (claro/escuro).
         A classe isLeaving aplica animacao de saida ao navegar para outra pagina. -->
    <div :class="[
        ...
        <!-- Elementos decorativos de fundo (circulos desfocados) — apenas visuais, sem interacao -->
        ...
        <!-- Area de conteudo principal centrada verticalmente -->
        <main class="relative mx-auto flex min-h-screen w-full max-w-6xl items-center px-4 py-10 sm:px-6 lg:px-8">
            <!-- Cartao principal: grelha de duas colunas em desktop (formulario | painel lateral) -->
            <section :class="[
                ...
                <div :class="[
                    ...
                    <!-- Logo com link de regresso à pagina inicial -->
                    <a href="/"
                        class="mx-auto inline-flex w-fit items-center gap-3 rounded-full bg-white/80 px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:text-blue-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-300 dark:bg-slate-900/80 dark:text-slate-200 dark:hover:text-blue-300"
                        aria-label="Voltar para a pagina inicial">
                        <img :src="'/image/logo/ToDo.png'" alt="To-Do App" class="h-10 sm:h-14 lg:h-16 w-auto">
                    </a>

                    <!-- Cabecalho textual da pagina de login -->
                    <div class="mt-6 sm:mt-8 max-w-md">
                        <p class="text-sm font-semibold uppercase tracking-[0.28em] text-blue-600 dark:text-blue-300">
                            Acesso ao To-Do App</p>
                        <h1 id="login-title"
                            class="mt-3 text-3xl font-black tracking-tight text-slate-900 dark:text-white sm:text-4xl lg:text-5xl">
                            Bem-vindo de volta</h1>
                        <p class="mt-4 text-sm leading-6 text-slate-600 dark:text-slate-300 sm:text-base">
                            Entre para gerir tarefas, acompanhar prioridades e manter o trabalho da equipa sempre
                            organizado.
                        </p>
                    </div>

                    <!-- Formulario: submete login normal ou verificacao 2FA consoante o estado -->
                    <form @submit.prevent="pendingTwoFactor ? submitTwoFactor() : submitLogin()" class="mt-8 space-y-5">
                        ...
                        <div>
                            ...
                            <div class="group relative">
                                <!-- Icone decorativo do campo email -->
                                <span
                                    class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 text-slate-400 transition group-focus-within:text-blue-500 dark:text-slate-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                        class="h-5 w-5" aria-hidden="true">
                                        <path
                                            d="M1.5 8.67v8.58A2.25 2.25 0 0 0 3.75 19.5h16.5a2.25 2.25 0 0 0 2.25-2.25V8.67l-9.41 5.56a2.25 2.25 0 0 1-2.28 0L1.5 8.67Z" />
                                        <path
                                            d="M22.5 6.908V6.75A2.25 2.25 0 0 0 20.25 4.5H3.75A2.25 2.25 0 0 0 1.5 6.75v.158l10.07 5.95a.75.75 0 0 0 .86 0L22.5 6.908Z" />
                                    </svg>
                                </span>
                        ...
                        <!-- Campo de senha com botao para mostrar/ocultar o texto -->
                        <div>
                            <label for="password"
                                class="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">Senha</label>
                            <div class="group relative">
                                <!-- Icone decorativo do campo senha -->
                                <span
                                    class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 text-slate-400 transition group-focus-within:text-blue-500 dark:text-slate-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                        class="h-5 w-5" aria-hidden="true">
                                        <path fill-rule="evenodd"
                                            d="M12 1.5a4.5 4.5 0 0 0-4.5 4.5V9H6a2.25 2.25 0 0 0-2.25 2.25v8.25A2.25 2.25 0 0 0 6 21.75h12a2.25 2.25 0 0 0 2.25-2.25v-8.25A2.25 2.25 0 0 0 18 9h-1.5V6A4.5 4.5 0 0 0 12 1.5Zm3 7.5V6A3 3 0 0 0 9 6V9h6Z"
                                            clip-rule="evenodd" />
                                    </svg>
                                </span>
                                ...
                                <!-- Botao toggle para alternar visibilidade da senha -->
                                <button type="button"
                                    ...
                                    <!-- Icone "olho riscado" quando a senha esta visivel -->
                                    <svg v-if="showPassword" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                        ...
                                        <path
                                            d="M16.68 16.69A8.7 8.7 0 0 1 12 18c-5 0-9-6-9-6a18.7 18.7 0 0 1 5.02-4.62">
                                        </path>
                                        <path
                                            d="M9.88 5.09A9.12 9.12 0 0 1 12 5c5 0 9 6 9 6a18.85 18.85 0 0 1-1.95 2.79">
                                        </path>
                                    </svg>
                                    <!-- Icone "olho aberto" quando a senha esta oculta -->
                        ...
                        <!-- Campo do codigo 2FA: visivel apenas quando o servidor pede verificacao adicional -->
                        <div v-if="pendingTwoFactor">
                            <label for="two-factor-code"
                                class="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">Código de
                                verificação</label>
                            <input id="two-factor-code" v-model="twoFactorCode" type="text" inputmode="numeric"
                            ...
                            <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">Verifique o código enviado para o
                                seu email.</p>
                        </div>

                        <!-- Opcao "lembrar de mim": oculta durante o fluxo 2FA -->
                        <div v-if="!pendingTwoFactor"
                            class="flex flex-col gap-3 text-sm text-slate-600 dark:text-slate-300 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <!-- Mensagem de erro exibida quando o login ou validacao 2FA falha -->
                        <p v-if="errorMessage"
                            class="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-300">
                            {{ errorMessage }}</p>

                        <!-- Botao principal de submissao: muda o texto consoante o estado (loading / 2FA) -->
                        <button type="submit" :disabled="isLoading"
                            ...
                            {{ isLoading ? (pendingTwoFactor ? 'A validar...' : 'Entrando...') : (pendingTwoFactor ?
                            'Validar código' : 'Entrar') }}
                        </button>

                        <!-- Botao de regresso ao formulario de login durante o fluxo 2FA -->
                        <button v-if="pendingTwoFactor" type="button"
                ...
                <!-- Coluna direita: painel de marketing com destaque para o registo -->
                <div :class="[
                    ...
                    <div class="flex w-full justify-center">
                        <!-- Cartao interior com glassmorphism -->
                        <div
                            class="relative max-w-md rounded-[2rem] border border-white/20 bg-white/10 p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.18)] backdrop-blur-sm sm:p-8">
                            <!-- Botao de fecho que navega com animacao de saida -->
                            <button type="button"
                                ...
                                aria-label="Fechar pagina" title="Fechar" @click="navigateWithTransition('/')">
                            ...
                            <!-- Textos de apresentacao do produto -->
                            <p class="text-sm font-semibold uppercase tracking-[0.3em] text-blue-100">To-Do App</p>
                            <h2 class="mt-4 text-2xl font-black tracking-tight sm:text-3xl lg:text-4xl">Organize tudo
                                num so lugar</h2>
                            <p class="mt-3 text-sm leading-6 text-blue-50/90 hidden sm:block">
                                Crie a sua conta para centralizar tarefas, definir prazos e acompanhar o progresso
                                diario sem confusao.
                            </p>

                            <!-- Cards de funcionalidades resumidas -->
                            ...
                            <!-- Link para a pagina de registo com animacao de transicao -->
                            <a href="/register" @click.prevent="navigateWithTransition('/register')"
        ...
        <!-- Botao flutuante para alternar entre tema claro e escuro -->
        <button id="theme-toggle" type="button"
            ...
            :aria-label="themeLabel" :aria-pressed="isDark" title="Alternar tema" @click="toggleTheme">
            <span class="sr-only">{{ themeLabel }}</span>
            <!-- Icone de sol (tema claro ativo) -->
            <svg v-show="!isDark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" class="h-5 w-5" aria-hidden="true">
            ...
            <!-- Icone de lua (tema escuro ativo) -->
            <svg v-show="isDark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" class="h-5 w-5" aria-hidden="true">
...
const email = ref('');
const password = ref('');
const remember = ref(false);       // Opcao "lembrar de mim"
const isLoading = ref(false);      // Controla o estado de carregamento durante o fetch
const errorMessage = ref('');      // Mensagem de erro a mostrar ao utilizador
const isDark = ref(false);         // Estado atual do tema (claro/escuro)
const isLeaving = ref(false);      // Ativa a animacao de saida ao navegar
const showPassword = ref(false);   // Controla a visibilidade do campo de senha
const pendingTwoFactor = ref(false); // True quando o servidor exige verificacao 2FA
const twoFactorCode = ref('');     // Codigo de 6 digitos introduzido pelo utilizador

// Token CSRF lido do meta tag injetado pelo Laravel (necessario para requisicoes POST)
const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';

// Label acessivel para o botao de tema, muda consoante o estado atual
const themeLabel = computed(() => (isDark.value ? 'Ativar tema claro' : 'Ativar tema escuro'));

// Sincroniza o estado local 'isDark' com a classe 'dark' no elemento <html>
function syncThemeState() {
    isDark.value = document.documentElement.classList.contains('dark');
}

// Listener para sincronizar o tema quando alterado noutro tab do browser
...
// Alterna o tema claro/escuro e persiste a escolha no localStorage
...
// Inicia a animacao de saida e navega para o URL indicado apos 360ms
function navigateWithTransition(url) {
    if (isLeaving.value) {
        return; // Evita navegacao dupla durante a animacao
    }
...
// Envia as credenciais de login para o servidor via fetch.
// Se o servidor responder com requires_two_factor, activa o fluxo 2FA.
// Caso contrario, redireciona para /todo em caso de sucesso.
async function submitLogin() {
    ...
    try {
        ...
        // O servidor pede verificacao 2FA: mostra o campo do codigo
        ...
        // Login bem-sucedido: redireciona para a pagina de tarefas
        if (response.ok) {
            window.location.href = data?.redirect || '/todo';
            return;
        }

        // Erro de autenticacao: mostra a mensagem devolvida pelo servidor
...
// Repoe o estado do fluxo 2FA (volta ao formulario de login normal)
...
// Envia o codigo 2FA para o servidor para validacao.
// Em caso de sucesso, redireciona para /todo.
async function submitTwoFactor() {
    ...
    try {
        ...
        // Codigo valido: redireciona para a pagina de tarefas
        if (response.ok) {
            window.location.href = data?.redirect || '/todo';
            return;
        }

        // Codigo invalido ou expirado: mostra erro
...
// Ao montar o componente: sincroniza o tema e regista o listener de storage
onMounted(() => {
    syncThemeState();
    window.addEventListener('storage', handleStorage);
});

// Ao desmontar o componente: remove o listener para evitar memory leaks
onUnmounted(() => {
